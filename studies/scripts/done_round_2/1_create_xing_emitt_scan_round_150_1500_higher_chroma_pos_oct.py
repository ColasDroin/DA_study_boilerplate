...
array_emitt = np.linspace(2.2, 3, 17, endpoint=True) * 1e-6
...
for idx_job, (track, emitt, xing) in enumerate(
    itertools.product(track_array, array_emitt, array_xing)
):
    ...
    for beam in ["lhcb1", "lhcb2"]:
        ...
        d_config_collider["config_beambeam"]["nemitt_x"] = float(emitt)
        d_config_collider["config_beambeam"]["nemitt_y"] = float(emitt)
        if xing < 190:
...
# ==================================================================================================
# Define study name
study_name = "xing_emitt_scan_round_150_1500_higher_chroma_pos_oct"
