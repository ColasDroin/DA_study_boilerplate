...
d_config_particles["n_angles"] = 20  # 5

# Number of split for parallelization
d_config_particles["n_split"] = 40  # 20
...
d_config_knobs["on_sep2v"] = 0  ##d_config_knobs["on_x5"] = 170
...
filling_scheme_path = os.path.abspath(
    "../filling_scheme/50ns_961b_880_880_209_40bpi_25inj_PbPb.json"
)
...
# ==================================================================================================
# Reduce crossing angles in steps

mydf = pd.read_parquet("for_sim_sofia_fig5.parquet")
array_onx1 = mydf["xing1"].values  # np.round(np.arange(170.0, 90.0, -5.0), decimals=4)
array_onx5 = mydf["xing5"].values  # np.round(np.arange(170.0, 90.0, -5.0), decimals=4)
array_onx2 = mydf["xing2"].values  # np.round(np.arange(170.0, 90.0, -5.0), decimals=4)
# array_onx2 = np.round(np.arange(-170.0, -90.0, 5.0), decimals=4)
array_onx8 = mydf["xing8"].values  # np.round(np.arange(-135.0, -55, 5.0), decimals=4)

array_intensity = mydf.intensity.values
array_bl = mydf.bl.values
...
for idx_job, (track, counter_on_x) in enumerate(
    itertools.product(track_array, np.arange(0, len(array_onx1)))
):
    # Mutate the appropriate collider parameters
    d_config_collider["config_knobs_and_tuning"]["knob_settings"]["on_x1"] = float(
        array_onx1[counter_on_x]
    )
    d_config_collider["config_knobs_and_tuning"]["knob_settings"]["on_x2v"] = float(
        array_onx2[counter_on_x]
    )
    d_config_collider["config_knobs_and_tuning"]["knob_settings"]["on_x5"] = float(
        array_onx5[counter_on_x]
    )
    d_config_collider["config_knobs_and_tuning"]["knob_settings"]["on_x8h"] = float(
        array_onx8[counter_on_x]
    )

    d_config_collider["config_beambeam"]["num_particles_per_bunch"] = float(
        array_intensity[counter_on_x]
    )
    d_config_collider["config_beambeam"]["sigma_z"] = float(array_bl[counter_on_x])
