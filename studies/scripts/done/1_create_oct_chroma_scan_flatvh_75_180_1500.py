...
for beam in ["lhcb1", "lhcb2"]:
    ...
    # Chroma will be scanned
    d_config_tune_and_chroma["dqx"][beam] = np.nan
    d_config_tune_and_chroma["dqy"][beam] = np.nan
...
# Octupoles
# Oct will be scanned
d_config_knobs["i_oct_b1"] = np.nan
d_config_knobs["i_oct_b2"] = np.nan
...
array_dq = np.linspace(3, 25, 23, endpoint=True)
array_oct = np.linspace(-600, 600, 49, endpoint=True)
...
for idx_job, (track, dq, octupoles) in enumerate(
    itertools.product(track_array, array_dq, array_oct)
):
    # Mutate the appropriate collider parameters
    for beam in ["lhcb1", "lhcb2"]:
        d_config_collider["config_knobs_and_tuning"]["dqx"][beam] = float(dq)
        d_config_collider["config_knobs_and_tuning"]["dqy"][beam] = float(dq)
        d_config_collider["config_knobs_and_tuning"]["knob_settings"]["i_oct_b1"] = float(octupoles)
        d_config_collider["config_knobs_and_tuning"]["knob_settings"]["i_oct_b2"] = float(octupoles)
