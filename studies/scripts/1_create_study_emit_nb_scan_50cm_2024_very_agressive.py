...
for beam in ["lhcb1", "lhcb2"]:
    d_config_tune_and_chroma["qx"][beam] = 62.318
    d_config_tune_and_chroma["qy"][beam] = 60.323
...
array_nb = np.round(np.linspace(1.5, 2.5, 11), decimals=4) * 1e8
array_emitt = np.round(np.linspace(1.5, 3, 16), decimals=4) * 1e-6
# ==================================================================================================
...
for idx_job, (track, nb, emit) in enumerate(itertools.product(track_array, array_nb, array_emitt)):
    # Mutate the appropriate collider parameters
    for beam in ["lhcb1", "lhcb2"]:
        d_config_collider["config_beambeam"]["num_particles_per_bunch"] = float(nb)
        d_config_collider["config_beambeam"]["nemitt_x"] = float(emit)
        d_config_collider["config_beambeam"]["nemitt_y"] = float(emit)
