...
def get_title_from_conf(
    ...
    if "hybrid" in conf_mad["optics_file"]:
        LHC_version = LHC_version + " (hybrid)"
    if "ions" in conf_mad["optics_file"] or "ION" in conf_mad["optics_file"]:
        LHC_version = LHC_version + " (ions)"
        ions = True
    else:
        ions = False
    ...
    if ions:
        energy = f"$E = {{{(energy_value/82):.1f}}}$ Z $TeV$"
    else:
...
def plot_heatmap(
    ...
    if plot_contours:
        CSS = ax.contour(
            ...
            mx,
            colors="black",
            levels=list(np.arange(int(vmin), green_contour, 0.5))
            + list(np.arange(green_contour + 0.5, int(vmax), 0.5)),
            linewidths=0.2,
        )
