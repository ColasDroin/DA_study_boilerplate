...
skip_leveling = True
# ! Skip leveling is True, so we ignore the following parameters
# # Leveling in IP 1/5
...
] = 1.8e11  # We start with max bunch intensity
d_config_beambeam["nemitt_x"] = 2.2e-6
d_config_beambeam["nemitt_y"] = 2.2e-6

# ! Skip beam-beam as it's implemented dynamically
...
d_config_simulation["n_turns"] = 25000
...
for idx_job, (track, config_bb) in enumerate(
    itertools.product(track_array, l_config_BB)
):
    ...
    #     if qy < (qx - 2 - 0.0039):  # 0.039 instead of 0.04 to avoid rounding errorss
    #         continue
