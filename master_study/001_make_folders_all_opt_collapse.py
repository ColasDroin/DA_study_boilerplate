...
array_optics = [
    "acc-models-lhc/strengths/round/start_collapse/opt_collapse_700_1500_thin.madx",
    "acc-models-lhc/strengths/round/start_collapse/opt_collapse_flathv_700_2800_thin.madx",
    "acc-models-lhc/strengths/round/start_collapse/opt_collapse_flathv_900_1800_1500_thin.madx",
]
d_config_mad["optics_file"] = None
...
children = {
    f"collider_{idx_optics}": {"config_particles": {}, "config_mad": {}, "children": {}}
    for idx_optics in range(len(array_optics))
}

for idx_optics, optics in enumerate(array_optics):
    # Add particles distribution parameters to the first generation
    children[f"collider_{idx_optics}"]["config_particles"] = d_config_particles

    d_config_mad["optics_file"] = optics

    # Add base machine parameters to the first generation
    children[f"collider_{idx_optics}"]["config_mad"] = copy.deepcopy(d_config_mad)
    ...
    for idx_job, (track, qx) in enumerate(itertools.product(track_array, array_qx)):
        ...
        children[f"collider_{idx_optics}"]["children"][f"xtrack_{idx_job:04}"] = {
