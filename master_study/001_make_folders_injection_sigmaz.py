...
array_sigmaz = np.round(np.linspace(0.07, 0.1, 31, endpoint=True), decimals=3)
print(array_sigmaz)
...
for idx_job, (track, sigmaz) in enumerate(itertools.product(track_array, array_sigmaz)):

    # Mutate the appropriate collider parameters
    d_config_collider["config_beambeam"]["sigma_z"] = float(sigmaz)
