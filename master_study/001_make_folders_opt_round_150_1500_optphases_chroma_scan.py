...
array_chroma = np.round(np.linspace(3, 20, 18, endpoint=True), decimals=0)
...
for idx_job, (track, qx, dqx) in enumerate(itertools.product(track_array, array_qx, array_chroma)):
    ...
    for beam in ["lhcb1", "lhcb2"]:
        ...
        d_config_collider["config_knobs_and_tuning"]["dqx"][beam] = float(dqx)
        d_config_collider["config_knobs_and_tuning"]["dqy"][beam] = float(dqx)
