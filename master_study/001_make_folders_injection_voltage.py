...
array_voltage = np.round(np.linspace(4, 12, 17, endpoint=True), decimals=2)
print(array_voltage)
...
for idx_job, (track, voltage) in enumerate(itertools.product(track_array, array_voltage)):

    # Mutate the appropriate collider parameters
    d_config_collider["config_knobs_and_tuning"]["knob_settings"]["vrf400"] = float(voltage)
