...
array_qx = np.round(np.arange(62.305, 62.330, 0.001), decimals=4)[:2]
array_qy = np.round(np.arange(60.305, 60.330, 0.001), decimals=4)[:1]
array_xing = [150, 155][:1]
...
for idx_optics, optics in enumerate(array_optics):
    ...
    for idx_job, (track, qx, qy, xing) in enumerate(
        itertools.product(track_array, array_qx, array_qy, array_xing)
    ):
        ...
        sign_on_x1 = np.sign(d_config_collider["config_knobs_and_tuning"]["knob_settings"]["on_x1"])
        sign_on_x5 = np.sign(d_config_collider["config_knobs_and_tuning"]["knob_settings"]["on_x5"])
