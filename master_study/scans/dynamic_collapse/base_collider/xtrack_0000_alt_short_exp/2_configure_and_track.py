    ...
    for i in range(n_steps + 1):
        ...
        if config_bb is not None:
            ...
            if i == 0:
                ...
                with open(
                    f"../xtrack_0000_identical_short_exp/bb_elements_step_{i}.pkl", "rb"
                ) as fid:
                    dic_elements = pickle.load(fid)

                # for beam_temp in ["lhcb1", "lhcb2"]:
                #     for element in dic_elements[beam_temp]:
                #         collider[beam_temp].element_dict[element] = copy.deepcopy(
                ...
                # print("Loading configuration from dictionnary")
                # with open(
                #     f"../xtrack_0000_identical_short_exp/bb_config_step_{i}.pkl", "rb"
                # ) as fid:
                #     dic_config = pickle.load(fid)
                # collider._bb_config = copy.deepcopy(dic_config)

                # for attr in dir(dic_elements[beam][element]):
                #     try:
                #         setattr(
                #             collider[beam][element],
                #             attr,
                #             getattr(dic_elements[beam][element], attr),
                #         )
                #         print(
                #             "Setting attribue"
                #             + attr
                #             + "to"
                #             + str(getattr(dic_elements[beam][element], attr))
                #             + "for element"
                #             + element
                #             + "in beam"
                #             + beam
