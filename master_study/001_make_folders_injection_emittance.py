...
d_config_mad["optics_file"] = (
    "acc-models-lhc/operation/optics/R2023a_A11mC11mA10mL10m_PhaseKnob100ON.madx"
)
d_config_mad["settings_file"] = (
    "acc-models-lhc/scenarios/pp_lumi/RAMP-SQUEEZE-6.8TeV-ATS-2m-2023_V1/0/settings.madx"
)
...
array_emittance = np.round(np.linspace(1.5, 3, 31, endpoint=True), decimals=2) * 1e-6
print(array_emittance)
...
for idx_job, (track, emittance) in enumerate(itertools.product(track_array, array_emittance)):

    # Mutate the appropriate collider parameters
    d_config_collider["config_beambeam"]["nemitt_x"] = float(emittance)
    d_config_collider["config_beambeam"]["nemitt_y"] = float(emittance)
